...
import { ICreatedGroupResponse, ICreateGroupRequest, IFriend, IFriendList } from '../../models/chat.model';
...
export class CreateGroupStateService {
  ...
  // Friend list state — managed directly (not via toSignal)
  filteredFriends = signal<IFriend[]>([]);
  friendsLoading = signal(false);
  friendsError = signal<string | null>(null);
  ...
  /** Call this to load/search friends. Pass '' to load all. */
  loadFriends(query: string = ''): void {
    this.friendsLoading.set(true);
    this.friendsError.set(null);

    this.friendService.searchFriend(query).pipe(
      catchError((err) => {
        this.logger.error('Failed to load friends:', err);
        this.friendsError.set('Failed to load friends. Please try again.');
        return of({ friends: { content: [] } } as unknown as IFriendList);
      }),
    ).subscribe((friendList) => {
      this.friendsLoading.set(false);
      this.filteredFriends.set(friendList?.friends?.content ?? []);
    });
  }
  ...
  addMember(friend: IFriend): void {
    if (this.selectedFriends().some(f => f.userId === friend.userId)) return;
