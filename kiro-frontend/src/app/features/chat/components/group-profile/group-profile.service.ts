...
export class GroupProfileService {
  ...
  // ── Update group name ──────────────────────────────────────────────────────
  updateGroupName(groupId: string, groupName: string): Observable<IGroupConversationMetaResponse> {
    return this.webApiService.patch<IGroupConversationMetaResponse>(
      `/groups/${groupId}`,
      { groupName }
    );
  }
  ...
  // ── Update group avatar ────────────────────────────────────────────────────
  updateGroupAvatar(groupId: string, file: File): Observable<IGroupConversationMetaResponse> {
    const formData = new FormData();
    formData.append('groupProfilePicture', file);
    return this.webApiService.patchForm<IGroupConversationMetaResponse>(
