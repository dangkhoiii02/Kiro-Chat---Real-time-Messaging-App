...
import com.kiro.kiro_backend.core.messaging.domain.repository.DirectMessagePublisher;
...
public class MessageTrackingStateService {
  ...
  public void markDirectMessageSeen(MarkMessageSeenRequest request) {
    ...
    Message message = messageRepository.getByPublicId(request.getMessagePublicId())
        .orElse(null);
    if (message == null) {
      log.warn("Omit seen state for missing direct message {}", request.getMessagePublicId());
      return;
    }
    ...
    try {
      ...
      log.warn("Omit seen state because user {} is not a participant of conversation {}",
          requestUser.getDbId(), message.getConversationId());
      return;
    }
  ...
  public void markGroupMessageSeen(MarkMessageSeenRequest request) {
    ...
    Message message = messageRepository.getByPublicId(request.getMessagePublicId())
        .orElse(null);
    if (message == null) {
      log.warn("Omit seen state for missing group message {}", request.getMessagePublicId());
      return;
    }
