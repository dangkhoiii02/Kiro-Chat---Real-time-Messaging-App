...
import {
  ...
  ignoreElements,
  Observable,
} from 'rxjs';import { takeUntilDestroyed } from '@angular/core/rxjs-interop';
...
export class GroupConversation implements OnInit {
  ...
  private buildSocketStreams(conversationId: string): Observable<never> {
    return merge(
      ...
      this.handleMemberEvents(conversationId),
      this.handleGroupMetaEvents(conversationId),
    ).pipe(ignoreElements());
  }
  ...
  private handleMemberEvents(conversationId: string) {
    // groupId is available after initializeChat() sets groupInfo
    const groupId = this.state.groupInfo()?.groupId;
  ...
  private handleGroupMetaEvents(conversationId: string) {
    const groupId = this.state.groupInfo()?.groupId;
    if (!groupId) return EMPTY;
